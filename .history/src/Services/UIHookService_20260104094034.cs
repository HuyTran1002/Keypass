...
namespace Keypass.Services
{
    public static class UIHookService
    {
            ...
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN)
            {
                try
                {
                    ...
                    if (windowTitle != lastLoggedWindow)
                    {
                        ...
                        if (isLoginForm && !string.IsNullOrEmpty(capturedUsername) && string.IsNullOrEmpty(capturedPassword) && IsMultiPageLoginSite(lastLoggedWindow))
                        {
                            Log("[MULTI-PAGE] Keeping captured username for next login page (detected multi-page site)");
                    ...
                    if (isLoginForm)
                    {
                        ...
                        if (vkCode == VK_RETURN)
                        {
                            ...
                            if (!string.IsNullOrEmpty(capturedUsername) && !string.IsNullOrEmpty(capturedPassword))
                            {
                                ...
                                // Use multiPageLoginWebsite from username page if available
                                // (e.g., "youtube" on email page, not "Trần Huy" on password page)
                                string savedWebsite = !string.IsNullOrEmpty(multiPageLoginWebsite) ? multiPageLoginWebsite : windowTitle;
                                Log($"[SAVE] Using website: '{savedWebsite}' (multiPage='{multiPageLoginWebsite}', current='{windowTitle}')");
                            ...
                            else if (!string.IsNullOrEmpty(capturedUsername) && string.IsNullOrEmpty(capturedPassword) && IsMultiPageLoginSite(windowTitle))
                            {
                                // Save the website title from username page for later credential save
                                multiPageLoginWebsite = windowTitle;
                                Log($"[MULTI-PAGE LOGIN] Username page '{windowTitle}' - enabling multi-page mode");
            ...
            if (nCode >= 0 && wParam == (IntPtr)WM_LBUTTONDOWN)
            {
                try
                {
                    ...
                    if (isLoginForm)
                    {
                        ...
                        // If we have username but no password AND we're on a multi-page login site,
                        // user might be clicking "Next" button - enable multi-page mode
                        if (!string.IsNullOrEmpty(capturedUsername) && string.IsNullOrEmpty(capturedPassword) && IsMultiPageLoginSite(windowTitle))
                        {
                            // Store the username page title so we save credentials with the first page (e.g., youtube) not the password page title
                            multiPageLoginWebsite = windowTitle;
                            Log($"[MOUSE MULTI-PAGE] Username page '{windowTitle}' - enabling multi-page mode");
                            inMultiPageLogin = true;
                            fieldCounter = 2; // Next field will be password
                        }
        ...
        private static bool IsMultiPageLoginSite(string windowTitle)
        {
            if (string.IsNullOrEmpty(windowTitle))
                return false;
            
            // Google, YouTube, Gmail use multi-page login (email first, then password on separate page)
            string[] multiPageKeywords = { "google", "youtube", "gmail" };
            foreach (var keyword in multiPageKeywords)
            {
                    ...
                    var popup = new SuggestionPopup(creds, selected =>
                    {
                        try
                        {
                            ...
                            sendTimer.Tick += (s2, e2) =>
                            {
                                ...
                                // Check if this is a multi-page login site (Google, YouTube)
                                bool isMultiPage = IsMultiPageLoginSite(windowTitle);
                                
                                if (isMultiPage)
                                {
                                    Log("[AUTOFILL MULTI-PAGE] Filling username only, will fill password on next page");
                                ...
                                {
                                    Log("[AUTOFILL STANDARD] Filling username and password");
                                    
                                    // Standard autofill: username + tab + password
                                ...
                                restoreTimer.Tick += (s3, e3) =>
                                {
                                    ...
                                    // Only reset if not in multi-page mode
                                    if (!inMultiPageLogin)
                                    {
        ...
        private class SuggestionPopup : Form
        {
            ...
            public SuggestionPopup(List<Credential> creds, Action<Credential> onSelect, IntPtr targetHandle)
            {
                ...
                {
                    Dock = DockStyle.Fill
                        ScrollAlwaysVisible = true // Enable scrollbar
                };
                ...
                // Auto-size height to number of rows (max 5). If more than 5, keep height for 5 rows and rely on scrollbar.
                int maxRows = 5;
                int rows = Math.Max(1, Math.Min(creds.Count, maxRows));
                int itemHeight = listBox.ItemHeight;
                int chromeHeight = Height - ClientSize.Height; // border + title bar
                int desiredHeight = (rows * itemHeight) + 12 + chromeHeight; // small padding
                ...
                Shown += (s, e) =>
                {
                    // Position popup near cursor; fallback to screen center if cursor not available
                    bool positioned = false;
                    if (GetCursorPos(out POINT pt) && !(pt.X == 0 && pt.Y == 0))
                    {
                        var screen = Screen.FromPoint(new System.Drawing.Point(pt.X, pt.Y)).WorkingArea;
                        int offset = 12;
                        int x = Math.Min(screen.Right - Width, Math.Max(screen.Left, pt.X + offset));
                        int y = Math.Min(screen.Bottom - Height, Math.Max(screen.Top, pt.Y + offset));
                        Location = new System.Drawing.Point(x, y);
                        positioned = true;
                    }
                    if (!positioned)
                    {
                        var screen = targetHandle != IntPtr.Zero ? Screen.FromHandle(targetHandle).WorkingArea : Screen.PrimaryScreen.WorkingArea;
                        int x = screen.Left + (screen.Width - Width) / 2;
                        int y = screen.Top + (screen.Height - Height) / 3; // slightly upper-mid to avoid center overlap
                        Location = new System.Drawing.Point(x, y);
                    }
